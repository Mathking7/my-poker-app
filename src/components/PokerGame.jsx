...
import { useTransitionCompletion } from '../hooks/useTransitionCompletion';
...
export default function PokerGame({ user, roomId, roomData, onLeaveRoom }) {
  ...
  const [chipEditMode, setChipEditMode] = useState('set');
  const [topUpAmount, setTopUpAmount] = useState(0);
  const [isLogOpen, setIsLogOpen] = useState(false);
  const [showMobileRaisePanel, setShowMobileRaisePanel] = useState(false);
  const [manualActionPendingKey, setManualActionPendingKey] = useState(null);
  const nowMs = useNowMs();
  const presentedTransition = usePresentedTransition({
    transition: roomData?.transition,
  ...
  useRoomPresence({ roomId, userUid: user?.uid });
  ...
  useEffect(() => {
    manualActionInFlightRef.current = null;
    setManualActionPendingKey(null);
  }, [roomData?.turnIndex, roomData?.status, roomData?.handCount, roomData?.currentBet, roomData?.pot, roomData?.updatedAt]);

  const myPlayerInfo = (roomData?.players || []).find(p => p.uid === user?.uid);
  const effectiveSettings = normalizeGameSettings(roomData?.settings);
  const pausedTransition = roomData?.isPaused && roomData?.transition?.pausedAt
    ? (presentedTransition?.id === roomData.transition.id ? presentedTransition : roomData.transition)
    : null;
  ...
  const actionPlayerUid = currentActionPlayer?.uid;
  const actionPlayerBet = currentActionPlayer?.bet;
  const actionPlayerChips = currentActionPlayer?.chips;
  const actionPlayerHasActed = currentActionPlayer?.hasActed;
  const actionPlayerFolded = currentActionPlayer?.folded;
  const actionPlayerAllIn = currentActionPlayer?.allIn;
  const currentManualActionKey = [
    roomId,
    roomData?.handCount,
    roomData?.status,
    roomData?.turnIndex,
  ...
  const isSeatAvailableForHand = (player, now = nowForRender) => Boolean(
    player &&
    !player.isKicked &&
    !player.isSittingOut &&
    !player.waitingNextHand &&
    (player.isAi || isPlayerActive(player, now, user?.uid, roomData))
  );
  const activeSeatedPlayers = (roomData?.players || []).filter((player) => isSeatAvailableForHand(player, nowForRender));
  const canManageRoom = Boolean(
    isHost ||
    (roomData?.isPublic && maintenanceManagerUid === user?.uid) ||
    (!roomData?.hostUid && isCreator)
  );
  const canEditPlayerChips = Boolean(isHost && roomData?.isPublic === false);
  const canStartGame = activeSeatedPlayers.length >= 2 && canManageRoom;
  ...
  const { timeLeft, timerProgress } = useTurnTimer({
    callAmount,
    currentActionNeedsInput,
    hasActiveTransition: Boolean(activeTransition),
    isMyTurn,
    isPaused: Boolean(roomData?.isPaused),
    isReferee,
    nowMs,
    onCurrentUserTimeout: (timeoutActionType) => handleAction(timeoutActionType),
    onRefereeTimeout: () => handleTimeoutForceAction(),
    resetKey: turnTimerResetKey,
    roomStatus: roomData?.status,
    timeLimit: effectiveSettings.timeLimit,
  });
  const showActionDock = Boolean(myPlayerInfo);
  const canTakeAction = !isManualActionPending && canPlayerTakeAction({
    isMyTurn,
    myPlayerInfo,
  ...
  const {
    ...
    currentPhaseInfo,
    effectiveSettings,
    isActionSubmitting: isManualActionPending,
    myPlayerInfo,
    roomData,
    timeLeft,
    userUid: user?.uid,
  ...
  useEffect(() => {
    let timeoutId;
    const now = Date.now();
    const seatedPlayers = (roomData?.players || []).filter((player) => isSeatAvailableForHand(player, now)).length;
  ...
  useEffect(() => {
    if (
      ...
      (!roomData.allInRunout && !shouldRevealAllInHands(roomData))
    ) {
      return undefined;
    }
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (
        ...
        (!currentRoom.allInRunout && !shouldRevealAllInHands(currentRoom))
      ) {
        return;
      }
      advanceGameState(currentRoom);
    }, 300);

    return () => clearTimeout(timeoutId);
    // advanceGameState is declared below; this effect only nudges stalled all-in states.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    activeTransition,
    canManageRoom,
  ...
  useTransitionCompletion({
    advanceGameState: (nextState) => advanceGameState(nextState),
    commitRoomState: (nextState, guard) => commitRoomState(nextState, guard),
    roomData,
    roomDataRef,
    roomId,
  ...
  const openPlayerManager = (player) => {
    if (!canManageRoom || (player?.uid === user?.uid && !canEditPlayerChips)) return;
    const pending = getPendingChipUpdate(player);
    setSelectedPlayer(player);
    setChipEditMode(pending?.mode || 'set');
    setTopUpAmount(pending?.amount ?? (pending?.mode === 'add' || pending?.mode === 'subtract' ? CHIP_UNIT : quantizeChipAmount(player?.chips || 0, 'floor')));
  };

  const commitRoomState = async (nextState, guard = null) => {
    if (!guard) {
      await setRoomDocument(roomId, nextState);
      return true;
    }

    return runRoomTransaction(roomId, async (transaction, roomRef) => {
      const snapshot = await transaction.get(roomRef);
      const latest = snapshot.exists() ? snapshot.data() : null;
      if (!guard(latest)) return false;
      transaction.set(roomRef, nextState);
      return true;
    });
  };

  const commitPlayerActionState = async (sourceState, actionType, amount = 0, options = {}) => {
    const sourceIndex = sourceState?.turnIndex;
    const sourceToken = getActionSourceToken(sourceState, sourceIndex);
    if (!sourceToken || (options.expectedUid && sourceToken.playerUid !== options.expectedUid)) return null;

    return runRoomTransaction(roomId, async (transaction, roomRef) => {
      const snapshot = await transaction.get(roomRef);
      const latest = snapshot.exists() ? snapshot.data() : null;
      if (!isSameActionSource(latest, sourceToken)) return null;

      const nextState = JSON.parse(JSON.stringify(latest));
      const playerIndex = nextState.turnIndex;
      nextState.transition = null;
      let applied = applyPlayerActionToState({ nextState, playerIndex, actionType, amount });
      if (!applied && options.fallbackRaiseToCall && actionType === 'raise') {
        applied = applyPlayerActionToState({ nextState, playerIndex, actionType: 'call', amount: 0 });
      }
      if (!applied) return null;

      const advanced = options.inlineAdvance !== false
        ? applyImmediatePostActionProgress(nextState)
        : false;

      transaction.set(roomRef, nextState);
      return { state: nextState, advanced };
    });
  };

  // ---------------- 房间管理操作 ----------------
  const handleAddAiPlayer = async () => {
    if (!canManageRoom || !roomData) return;
    const now = Date.now();
    ...
    if ((nextData.players || []).length >= MAX_PLAYERS) {
      nextData.logs = addLog(nextData, '⚠️ 房间人数已满，无法加入 AI。');
    ...
    if (isMidHand && !settings.allowJoinDuringGame) {
      nextData.logs = addLog(nextData, '⚠️ 当前设置不允许中途加入，AI 将在本局结束后再加入。');
    }

    const aiPlayer = createAiPlayer({
      existingPlayers: nextData.players || [],
    ...
    if (isMidHand) {
      ...
      aiPlayer.waitingNextHand = true;
    }

    nextData.players = [...(nextData.players || []), aiPlayer];
    nextData.logs = addLog(nextData, `🤖 ${aiPlayer.name} 加入房间${isMidHand ? '，将在下一局入座' : ''}。`);
    nextData.updatedAt = now;
    await setRoomDocument(roomId, nextData);
  };
  ...
  const handleTogglePause = async () => {
    if (!isHost) return;
    const now = Date.now();
    const willPause = !roomData.isPaused;
    const nextTransition = willPause
      ? pauseTransitionClock(roomData.transition, now)
      : resumeTransitionClock(roomData.transition, now);
    const patch = {
      isPaused: willPause,
      updatedAt: now,
    ...
    if (roomData.transition || nextTransition) {
      patch.transition = !willPause && nextTransition
        ? {
            ...nextTransition,
            pausedAt: deleteFieldValue(),
            pausedRemainingMs: deleteFieldValue(),
            pausedProgress: deleteFieldValue(),
          }
        : nextTransition;
    }
    await setRoomDocument(roomId, patch, { merge: true });
  };
  ...
  const handleResetGame = async () => {
    ...
    nextData.players = nextData.players.map(p => {
      ...
      clearPendingChipUpdate(nextPlayer);
      return nextPlayer;
    });
  ...
  const handlePlayerActionMenu = async (actionType) => {
    if (!canManageRoom || !selectedPlayer) return;
    let nextData = JSON.parse(JSON.stringify(roomData));
    ...
    if (actionType === 'kick') {
      ...
      if (!canEditPlayerChips) return;
      if (targetIndex === -1) return;
      const targetPlayer = nextData.players[targetIndex];
      const mode = chipEditMode === 'add' ? 'add' : (chipEditMode === 'subtract' ? 'subtract' : 'set');
      const amount = quantizeChipAmount(topUpAmount, 'floor');
      if ((mode === 'add' || mode === 'subtract') && amount <= 0) {
        alert(mode === 'add' ? '增加值必须大于 0。' : '减少值必须大于 0。');
        return;
      }
      targetPlayer.pendingChipMode = mode;
      targetPlayer.pendingChipAmount = amount;
      targetPlayer.pendingChipUpdatedAt = Date.now();
      nextData.logs = addLog(nextData, `💰 房主已为 ${selectedPlayer.name} 排队筹码调整：${getPendingChipLabel({ mode, amount })}，将在下一局开始前应用。`);
    }
    else if (actionType === 'clearChipUpdate') {
      if (!canEditPlayerChips) return;
      if (targetIndex === -1) return;
      const targetPlayer = nextData.players[targetIndex];
      const pending = getPendingChipUpdate(targetPlayer);
      if (!pending) return;
      clearPendingChipUpdate(targetPlayer);
      nextData.logs = addLog(nextData, `💰 房主取消了 ${selectedPlayer.name} 的筹码调整。`);
    }
    nextData.updatedAt = Date.now();
    await setRoomDocument(roomId, nextData);
  ...
  const handleLeave = async () => {
    if (myPlayerInfo) {
      ...
      if (me && isMidHand) {
        ...
        if (isHost) {
          const nextHost = nextData.players.find(p => p.uid !== user.uid && !p.isSittingOut && !p.isKicked && !p.isAi);
  ...
  const startGame = async () => {
    ...
    let players = roomData.players.filter(p => !p.isKicked).map(p => {
      const isUnavailable = p.isSittingOut || (!p.isAi && !isPlayerActive(p, now, user.uid, roomData));
      let currentChips = quantizeChipAmount(p.chips, 'floor');
      const pendingChipUpdate = roomData.isPublic ? null : getPendingChipUpdate(p);
      if (pendingChipUpdate) {
        const oldChips = currentChips;
        currentChips = pendingChipUpdate.mode === 'set'
          ? pendingChipUpdate.amount
          : (pendingChipUpdate.mode === 'subtract'
            ? Math.max(0, quantizeChipAmount(currentChips - pendingChipUpdate.amount, 'floor'))
            : quantizeChipAmount(currentChips + pendingChipUpdate.amount, 'floor'));
        logs.push(`💰 ${p.name} 的筹码调整已应用：${oldChips} → ${currentChips}。`);
      }
      if (!pendingChipUpdate && currentChips <= 0 && settings.autoTopUp && !isUnavailable) {
    ...
    if (activeSeatIndexes.length < 2) {
       if (roomData.status !== 'waiting') {
         await setRoomDocument(roomId, { ...roomData, players, settings, status: 'waiting', updatedAt: now });
       }
       return;
    }
  ...
  const advanceGameState = async (currentState) => {
    ...
    const holdLastActionBeforeAdvance = async () => {
      ...
      await commitRoomState(nextState, shouldCommitTransitionState(nextState));
    };
    const revealAllInHandsBeforeRunout = async () => {
      const revealNow = Date.now();
      nextState.allInRunout = true;
      nextState.turnIndex = -1;
      nextState.updatedAt = revealNow;
      ...
      nextState.transition = createGameTransition({
        id: `${nextState.handCount || 0}:all-in-reveal:${nextState.status}`,
        type: 'action-hold',
        fromStatus: nextState.status,
        toStatus: nextState.status,
        now: revealNow,
        message: '全下亮牌，准备发出剩余公共牌',
    ...
    if (nextState.status === 'showdown') {
      ...
      nextState.transition = nextState.transition || createGameTransition({
        id: `${nextState.handCount || 0}:showdown:river`,
        type: 'showdown',
        fromStatus: 'river',
        toStatus: 'showdown',
      ...
      await commitRoomState(nextState, shouldCommitSettlementState(nextState));
      return; 
    }
    ...
    if (activeContenders.length === 1) {
      ...
      nextState.transition = createGameTransition({
        id: `${nextState.handCount || 0}:showdown:${currentState.status}`,
        type: 'showdown',
    ...
    if (mustWait) {
      ...
      await commitRoomState(nextState, shouldCommitGameProgressState(nextState));
      return;
    }
    ...
    if (!nextState.allInRunout && shouldRevealAllInHands(nextState)) {
      await revealAllInHandsBeforeRunout();
      return;
    }
    ...
    if (nextState.status === 'pre-flop') {
      ...
      nextState.status = 'showdown';
      nextState.transition = createGameTransition({
        id: `${nextState.handCount || 0}:showdown:${fromStatus}`,
        type: 'showdown',
        fromStatus,
        toStatus: 'showdown',
        now: transitionNow,
    ...
    const shouldAutoRun = Boolean(nextState.allInRunout || shouldRevealAllInHands(nextState));
    if (shouldAutoRun) {
      nextState.allInRunout = true;
      nextState.players.forEach((player) => {
        if (!player.folded && player.hand?.length) {
    ...
    if (activeContenders.length >= 2) {
      nextState.transition = createGameTransition({
        id: `${nextState.handCount || 0}:street:${fromStatus}:${nextState.status}`,
        type: 'street',
        fromStatus,
        toStatus: nextState.status,
        now: transitionNow,
  ...
  const handleAction = async (actionType, amount = 0) => {
    ...
    if (manualActionInFlightRef.current === currentManualActionKey) return;

    manualActionInFlightRef.current = currentManualActionKey;
    setManualActionPendingKey(currentManualActionKey);
    setShowMobileRaisePanel(false);

    let keepPendingUntilRoomUpdate = false;
    try {
      const actionCommit = await commitPlayerActionState(roomData, actionType, amount, {
      ...
      if (actionCommit?.state) {
        ...
        keepPendingUntilRoomUpdate = true;
      }
    } finally {
      if (!keepPendingUntilRoomUpdate && manualActionInFlightRef.current === currentManualActionKey) {
        manualActionInFlightRef.current = null;
        setManualActionPendingKey(null);
      }
    }
  ...
  const selectedPlayerLive = selectedPlayer
    ? (roomData?.players || []).find((player) => player.uid === selectedPlayer.uid) || selectedPlayer
    : null;
  const selectedPendingChipUpdate = getPendingChipUpdate(selectedPlayerLive);
  ...
  return (
    <div className={`poker-game-shell ${statusClass} ${showActionDock ? 'poker-has-action' : ''} ${isMobileRaiseOpen ? 'poker-mobile-raise-open' : ''} h-[100dvh] bg-slate-900 text-slate-200 font-sans flex flex-col relative overflow-hidden`}>
      ...
      <PokerHeader
        ...
        canAddAi={(roomData.players || []).length < MAX_PLAYERS}
        onCopyRoomId={() => {
      ...
      <div className="poker-main-area flex-1 flex flex-col md:flex-row overflow-hidden relative">
        <div className="poker-table-area flex-1 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-emerald-800 to-slate-900 flex flex-col relative overflow-hidden">
          ...
          <div className="poker-table-scroll flex-1 overflow-y-auto flex flex-col p-4">
            {/* 对手头像 */}
            <div className="poker-opponents-strip" aria-label="对手座位，可横向滑动">
              <div className="poker-opponents-row flex justify-center gap-4 md:gap-8 flex-wrap z-0 flex-shrink-0">
                {roomData.players.map((p, idx) => {
                  ...
                  const isTurn = roomData.status !== 'waiting' && roomData.status !== 'showdown' && roomData.turnIndex === idx && playerNeedsAction(p, roomData) && !roomData.isPaused && !isActionLocked;
                  ...
                  return (
                    <OpponentCard
                      ...
                      onManagePlayer={openPlayerManager}
                    />
                  );
                })}
              </div>
          ...
          {myPlayerInfo ? (
            <SelfPlayerPanel
              ...
              canEditPlayerChips={canEditPlayerChips}
              isHost={isHost}
              isMyTurn={isMyTurn}
              ...
              onManageSelf={() => openPlayerManager(myPlayerInfo)}
      ...
      {selectedPlayerLive && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl shadow-2xl w-full max-w-sm border border-slate-600 overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-4 border-b border-slate-700 bg-slate-900/50">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><Crown size={18} className="text-amber-400"/> 管理玩家: {selectedPlayerLive.name}</h2>
            ...
            <div className="p-6 space-y-6">
              <div>
                <div className="mb-3 flex items-center justify-between gap-3 text-sm">
                  <span className="text-slate-400">当前筹码</span>
                  <span className="font-mono font-bold text-emerald-300">{selectedPlayerLive.chips}</span>
                </div>
                {canEditPlayerChips && selectedPendingChipUpdate && (
                  <div className="mb-3 rounded-lg border border-amber-500/40 bg-amber-950/30 px-3 py-2 text-sm text-amber-200">
                    已排队：{getPendingChipLabel(selectedPendingChipUpdate)}，下一局开始前应用
                  </div>
                )}
                {canEditPlayerChips ? (
                  <>
                    <div className="mb-3 grid grid-cols-3 rounded-lg border border-slate-700 bg-slate-900 p-1">
                      <button
                        onClick={() => {
                          setChipEditMode('set');
                          setTopUpAmount(selectedPendingChipUpdate?.mode === 'set' ? selectedPendingChipUpdate.amount : quantizeChipAmount(selectedPlayerLive.chips || 0, 'floor'));
                        }}
                        className={`rounded-md py-2 text-sm font-bold transition ${chipEditMode === 'set' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
                      >
                        设为
                      </button>
                      <button
                        onClick={() => {
                          setChipEditMode('add');
                          setTopUpAmount(selectedPendingChipUpdate?.mode === 'add' ? selectedPendingChipUpdate.amount : CHIP_UNIT);
                        }}
                        className={`rounded-md py-2 text-sm font-bold transition ${chipEditMode === 'add' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                      >
                        增加
                      </button>
                      <button
                        onClick={() => {
                          setChipEditMode('subtract');
                          setTopUpAmount(selectedPendingChipUpdate?.mode === 'subtract' ? selectedPendingChipUpdate.amount : CHIP_UNIT);
                        }}
                        className={`rounded-md py-2 text-sm font-bold transition ${chipEditMode === 'subtract' ? 'bg-rose-600 text-white' : 'text-slate-400 hover:text-white'}`}
                      >
                        减少
                      </button>
                    </div>
                    <label className="block text-sm text-slate-400 mb-2">
                      {chipEditMode === 'set' ? '下一局筹码设为' : (chipEditMode === 'subtract' ? '下一局开始前减少' : '下一局开始前增加')}
                    </label>
                    <div className="flex gap-2">
                      <input type="number" min={chipEditMode === 'set' ? '0' : CHIP_UNIT} step={CHIP_UNIT} value={topUpAmount} onChange={e => setTopUpAmount(quantizeChipAmount(e.target.value, 'floor'))} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2 text-white outline-none focus:border-emerald-500 font-mono" />
                      <button onClick={() => handlePlayerActionMenu('setChips')} className="px-5 bg-emerald-600 hover:bg-emerald-500 rounded-lg font-bold text-sm whitespace-nowrap shadow">排队应用</button>
                    </div>
                    {selectedPendingChipUpdate && (
                      <button onClick={() => handlePlayerActionMenu('clearChipUpdate')} className="mt-3 w-full rounded-lg border border-slate-600 py-2 text-sm font-bold text-slate-300 transition hover:border-rose-400 hover:text-rose-200">
                        取消待应用调整
                      </button>
                    )}
                  </>
                ) : (
                  <div className="rounded-lg border border-slate-700 bg-slate-900/70 px-3 py-2 text-sm text-slate-400">
                    公开房间不支持手动调整玩家筹码。
                  </div>
                )}
              </div>
              ...
              {selectedPlayerLive.uid !== user.uid && (
                <div className="border-t border-slate-700 pt-6 flex gap-3">
                  {isHost && !selectedPlayerLive.isAi && <button onClick={() => handlePlayerActionMenu('transfer')} className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 rounded-lg font-bold text-sm transition shadow">转让房主</button>}
